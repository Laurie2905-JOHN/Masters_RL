...
def reward_nutrient_macro(self, action):
    ...
    for nutrient, average_value in nutrient_values.items():
        ...
        if target_min != 0 and target_max != float('inf'):
            distance = max(abs(average_value - target_min), abs(average_value - target_max)) * 2 # Double the distance for calories as important
            far_flag = average_value < 0.2 * target_min or average_value > 5 * target_max
        elif target_min == 0:
            ...
            far_flag = average_value < 0.2 * target_min
        else:
            distance = 0
            far_flag = False
    ...
    if all_targets_met:
        ...
        self.termination_reason = 0 # 0 for no termination
    # Include the step penalty in the reward calculation
