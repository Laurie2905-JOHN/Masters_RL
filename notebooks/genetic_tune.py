...
def objective(trial):
    ...
    ngen = trial.suggest_int('ngen', 10, 200)  # Number of generations
    population_size = trial.suggest_int('population_size', 50, 1000)  # Population size
    cxpb = trial.suggest_float('cxpb', 0.3, 0.9)  # Crossover probability
    mutpb = trial.suggest_float('mutpb', 0.01, 0.5)  # Mutation probability
    ...
    for run in range(5):
        ...
        optimized_menu, score = menu_generator_simple.run_genetic_menu_optimization_and_finalize(
