...
# Predefined possible batch sizes
PREDEFINED_BATCH_SIZES = [32, 64, 128, 256, 512, 1024, 2048]

def ppo_objective(trial, n_envs, num_timesteps, num_seeds=5):
    n_steps = trial.suggest_int('n_steps', 16, 2048)

    # Filter possible batch sizes based on the constraint
    possible_batch_sizes = [bs for bs in PREDEFINED_BATCH_SIZES if (n_steps * n_envs) % bs == 0]

    # If no valid batch size is available, raise an exception to skip the trial
